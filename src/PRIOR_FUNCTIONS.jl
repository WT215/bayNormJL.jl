...
function EstPrior(;Data, verbose = true) 
    ...
    #temp_result_est=SharedArray{Float64,2}((size(Data)[1],2))
    # @sync begin
    ...
    #     @async begin
            # QQ=@distributed vcat for gene_ind=1:size(Data)[1]
            #     #println(gene_ind);
            #     vals=Data[gene_ind,:];
            #     n = length(vals);
            #     m = mean(vals);
            ...
            #     end
            #     estimate=[mme_size m];
            #     #temp_result_est[gene_ind,:]=estimate
            #     return estimate
            # #put!(channel, true)
            # end
...
function BB_Fun(;Data, BETA_vec, INITIAL_MU_vec, INITIAL_SIZE_vec, MU_lower = 0.01, MU_upper = 1000, SIZE_lower = 0.01,SIZE_upper = 100,FIX_MU = true)
    ...
    QQ=SharedArray{Float64,2}((size(Data)[1],2))
    #sum(temp_result[:,1].==0)
    if FIX_MU
        @sync begin    
            # this task prints the progress bar
            ...
            # this task does the computation
            @async begin
                @distributed vcat for gene_ind in range(1, stop = nRow)
                    #println(gene_ind)
                    #gene_ind=4086
            ...
            @async begin
                @distributed vcat for gene_ind in range(1, stop = nRow)
                    ...
                    if input_mme[2]!=0
                        ...
                        QQ[gene_ind,:]=best_candidate(res)
                        #best_candidate(res);
                    else
                        QQ[gene_ind,:]=input_mme
                        #input_mme;
